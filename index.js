...
// --- פונקציית הניקוי "החכמה" ---
function cleanTextMaster(text) {
    if (!text) return "";
    let clean = String(text);

    // 1. הסרת החתימה של ישיב'ע זוכע'ר (כל הבלוק)
    const signatures = [
        /'ישיב'ע זוכע'ר' - סקופים בלעדיים מעולם הישיבות/g,
        /chat\.whatsapp\.com\/[^\s]+/g,
        /ישיב׳ע זוכע’ר בצ׳אט/g,
        /https:\/\/yeshiva-zucher\.chatfree\.app/g,
        /לשליחת עדכונים לדסק ישיב’ע זוכע’ר/g,
        /wa\.me\/[^\s]+/g
    ];
    signatures.forEach(sig => clean = clean.replace(sig, ""));

    // 2. הסרת תמונות/סרטונים/קבצים (שמות קבצים נפוצים)
    clean = clean.replace(/\b[\w\-]+\.(jpg|jpeg|png|gif|mp4|pdf|heic)\b/gi, "");
    clean = clean.replace(/\[?(image|video|poll|file|תמונה|סרטון)\]?/gi, "");

    // 3. הסרת כל קישור HTTP/HTTPS שנשאר
    clean = clean.replace(/https?:\/\/[^\s]+/gi, "");

    // 4. ניקוי רווחים כפולים, סימנים מיותרים וסגירה בנקודה
...
async function run() {
    ...
    for (const src of sources) {
        try {
            ...
            // הפיכת סדר כדי להעלות מהישן לחדש
