...
const FORCE_REUPLOAD = true; // שנה ל-true  ולהחזיר ל: false אם אתה רוצה לנקות הכל ולהעלות מחדש פעם אחת
...
// פונקציית חילוץ שעה משופרת
function extractTime(m) {
    let textContent = String(m.text || m.content || m.description || "");
    let textMatch = textContent.match(/(\d{2}:\d{2})/);
    if (textMatch) return textMatch[1];

    // חיפוש שדה תאריך (תמיכה ביותר פורמטים)
    let rawTime = m.time || m.timestamp || m.createdAt || m.created_at || m.date || "";
    if (!rawTime) return new Date().toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Jerusalem' });

    let dateObj = new Date(rawTime);
    
    // אם התאריך לא תקין, ננסה לבדוק אם זה מספר (Unix Timestamp)
    if (isNaN(dateObj.getTime()) && /^\d+$/.test(rawTime)) {
        dateObj = new Date(Number(rawTime) * 1000);
        if (dateObj.getFullYear() < 2020) dateObj = new Date(Number(rawTime));
    }
...
async function run() {
    ...
    const sources = [
        ...
        // שינוי הכתובת של אותנטי ללא last_id כדי לקבל את החדשים ביותר
        { name: "אותנטי", url: "https://authenti.newsupdates.click/api/get_messages_optimized.php?source=ALL_CHANNELS&limit=50", folder: "3", key: "auth", useCookie: false },
        ...
        try {
            ...
            for (const m of msgs) {
                ...
                if (cleanTxt.length > 5) {
                    let finalString = isYeshiva ? `${time}:בעדכוני יְשִׁיבְע זּוּכֶער: ${cleanTxt}` :
                                     (src.key === "giz" ? `הגיזרה בשעה: ${time}, ${cleanTxt}` : 
                                     `${m.source || src.name} בשעה: ${time}, ${cleanTxt}`);
